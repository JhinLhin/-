...
> ------
>
> ## 一、DynamoDB 是什么？一句话记住它
>
> > **DynamoDB = AWS 提供的全托管、高可扩展、键值型 NoSQL 数据库**
>
> 三个关键词拆解：
>
> - **全托管**：不用你管服务器、扩容、备份，AWS 全包了
> - **高可扩展**：自动分片，流量再大也能扛，无需停机
> - **键值/NoSQL**：不是传统关系型数据库，schema 灵活，不需要提前定义表结构
>
> **面试用语**：*"我会选 DynamoDB，因为它全托管、自动扩容，支持事务，单位毫秒级延迟，几乎满足大部分持久层需求。"*
>
> ------
>
> ## 二、数据模型（三层结构）
...
> | DynamoDB 概念     | 类比              | 说明                                   |
> | ----------------- | ----------------- | -------------------------------------- |
> | Table（表）       | 一张 Excel 工作表 | 顶层数据结构                           |
> | Item（项目）      | 工作表中的一行    | 最多 400KB，必须有主键                 |
> | Attribute（属性） | 一个单元格        | 键值对，可嵌套，不同 item 可有不同属性 |
>
> **Schema-less（无模式）**：同一张表里，不同 item 可以有完全不同的字段，比如：
>
> ```
> Item 1: {PersonID: 101, LastName: "Smith", Phone: "555-4321"}
> Item 2: {PersonID: 102, LastName: "Jones", FavoriteColor: "Blue"}
> ```
>
> → 灵活，但数据校验需要在应用层自己做！
>
...
>
> ```
> 模式一：只有 Partition Key（哈希键）
> 模式二：Partition Key + Sort Key（复合主键）
> ```
>
> ### Partition Key（分区键）
>
> - 决定数据**存在哪个物理节点**（底层：哈希后映射到具体节点）
> - 选择原则：**高基数（cardinality）、均匀分布、匹配最常见查询**
> - 避免热点：如果 partition key 取值太集中，会导致某个节点过热
>
> ### Sort Key（排序键）
>
> - 同一个 partition key 下的 item 按 sort key **排序存储**（底层用 B-Tree）
> - 支持范围查询：`>`, `<`, `BETWEEN`, `begins_with`
> - 可选，但需要范围查询时必须有
>
> ### 面试经典例子：群聊消息表
>
> ```
> Partition Key: chat_id     → 快速找到某个群的所有消息
> Sort Key:      message_id  → 按时间顺序排列消息
> ```
>
> **为什么用 message_id 而不是 timestamp？**
>
> > 时间戳不唯一（同一毫秒可能有多条消息），message_id 既能排序又能保证唯一性。 推荐用：**UUID v7** 或 **Snowflake ID**（天然有序）
>
> ------
>
> ## 四、底层原理（理解，不背数字）
...
>
> ```
> 用户请求 → Request Router → 查 Partition Metadata Service → 找到对应存储节点
> ```
>
> - Partition Key 的值被哈希 → 决定存在哪个 partition
> - 类似一致性哈希，但 DynamoDB 用**中心化**的 partition map（不是 P2P 哈希环）
>
> ### 分区内部：B-Tree 索引
>
> - 每个 partition 内部用 B-Tree 按 Sort Key 排序存储
> - 使范围查询高效
>
> ### 每个 Partition 的容量上限（面试估算用）
>
> | 指标   | 上限                         |
> | ------ | ---------------------------- |
> | 读吞吐 | 3,000 RCU/s → 最多 12MB/s 读 |
> | 写吞吐 | 1,000 WCU/s → 最多 1MB/s 写  |
>
> → 超过限制，DynamoDB 自动**分裂 partition**，无需人工干预
>
> ------
>
> ## 五、Secondary Index（二级索引）
>
> **使用场景**：当你需要用**非主键属性**来查询时
>
> ### GSI（全局二级索引）—— 最常用
>
> <img src="C:\Learning Notes\Java\系统设计之神\img\image-20260228015306677.png" alt="image-20260228015306677" style="zoom: 50%;" />
>
> - 可以用**完全不同的 Partition Key** 建索引
> - 存储在**独立的分区**上（相当于内部维护了一张新表）
> - 更新异步（最终一致性）
> - 可以**随时创建和删除**
> - 每张表最多 20 个
>
> **群聊例子**：主表按 chat_id 分区。但用户想看"我发过的所有消息"，需要按 user_id 查 → 建 GSI：
>
> ```
> GSI Partition Key: user_id
> GSI Sort Key:      message_id
> ```
>
> ### LSI（本地二级索引）—— 较少用
>
> <img src="C:\Learning Notes\Java\系统设计之神\img\image-20260228015428090.png" alt="image-20260228015428090" style="zoom:50%;" />
>
> - Partition Key **必须和主表相同**，只换 Sort Key
> - 存在**同一个 partition** 里（效率更高，但受限更多）
> - 更新同步（支持强一致读）
> - **只能在建表时创建，之后无法添加删除**
> - 每张表最多 5 个，每个 partition key 下数据上限 10GB
>
> **群聊例子**：已经能按 message_id 排序，但想按附件数量 num_attachments 排序 → 建 LSI：
>
> ```
> LSI Partition Key: chat_id（与主表相同）
...
> |               | GSI              | LSI              |
> | ------------- | ---------------- | ---------------- |
> | Partition Key | 可以不同         | 必须和主表相同   |
> | 数据位置      | 独立分区         | 同一分区         |
> | 更新方式      | 异步（最终一致） | 同步（可强一致） |
> | 创建时机      | 随时             | 只能建表时       |
> | 强一致读      | ❌                | ✅                |
>
> ------
...
> | 操作      | 说明                        | 性能               |
> | --------- | --------------------------- | ------------------ |
> | **Query** | 用主键或索引键精确/范围查询 | ✅ 高效，**首选**   |
> | **Scan**  | 扫描整张表的每一条 item     | ❌ 慢，**尽量避免** |
>
> **重要**：`ProjectionExpression`（只返回部分字段）只减少网络带宽，**不减少 RCU 消耗**——DynamoDB 仍然读取整个 item！
>
> → 所以要合理**规范化数据**，避免把大量不常用数据堆在同一个 item 里（比如把 reviews 拆到单独的表）
>
> ------
>
> ## 七、一致性模型（CAP 相关）
>
> DynamoDB 可以**按请求级别**选择一致性：
>
> ### 最终一致性（默认）
>
> - 读请求可能打到三个副本中的任意一个（follower 可能略有延迟）
> - 消耗：**0.5 RCU / 4KB**
> - 延迟低，可用性高 → **AP 系统**
>
> ### 强一致性（ConsistentRead=true）
>
> - 读请求一定打到 **leader 节点**（leader 总有最新数据）
> - 消耗：**1 RCU / 4KB**（贵一倍）
> - 延迟略高
>
> **⚠️ 重要限制**：GSI **只支持最终一致性**，强一致读只能用于主表和 LSI！
>
> ### 底层架构：Multi-Paxos
>
> ```
> 每个 partition = 1个 leader + 2个 follower（跨3个可用区）
> 写操作：leader 先写 WAL，2/3 节点确认 → 返回成功（Quorum Write）
> 强一致读：请求路由到 leader
> 最终一致读：任意副本都可以响应
> ```
>
...
> DynamoDB **支持 ACID 事务**！
>
> - `TransactWriteItems`：原子性地写多个 item（最多 100 条，可跨多张表）
> - `TransactGetItems`：原子性地读多个 item
>
> → 彻底打破了"NoSQL 不支持事务"的刻板印象！
>
> ------
>
> ## 八、容量单位（RCU/WCU）
...
> | 单位  | 等值                                                   |
> | ----- | ------------------------------------------------------ |
> | 1 RCU | 强一致：1次读/s，最多 4KB；最终一致：2次读/s，最多 4KB |
> | 1 WCU | 1次写/s，最多 1KB（不足 1KB 也按 1KB 算，向上取整）    |
>
> **面试估算例子**：设计 YouTube 点赞系统，每秒 10,000,000 次写 → 每次写 ≤ 1KB → 1 WCU/次 → 需要 10,000,000 WCU → 约 10,000 个 partition → 成本巨大 → 此时可以考虑批量聚合后再写入来降低成本
>
> ------
>
> ## 九、高级特性（面试加分）
>
> ### DAX（DynamoDB Accelerator）—— 内置缓存
>
> - **无需引入 Redis**，DAX 就是 DynamoDB 专属的内存缓存
> - 延迟从毫秒级降到**微秒级**
> - 读写穿透缓存（Write-Through + Read-Through）
> - **⚠️ 注意**：强一致读不会被缓存，直接透传到 DynamoDB
>
> ### DynamoDB Streams —— 内置 CDC
>
> - 任何 item 的增删改都会产生一条 stream record
> - 可以触发 **Lambda** 函数（实时响应数据变化）
> - 可以同步数据到 **Elasticsearch**（实现搜索功能）
> - 可以接入 **Kinesis Data Streams** → Firehose → S3/Redshift（实时分析）
>
> ------
>
> ## 十、何时用 DynamoDB，何时不用
...
> | 场景                        | 原因                       | 替代方案     |
> | --------------------------- | -------------------------- | ------------ |
> | 复杂 JOIN / Ad-hoc 聚合查询 | 不支持 JOIN，查询模式受限  | PostgreSQL   |
> | 超高写入量（数十万/秒）     | 成本极高                   | 评估其他方案 |
> | 避免厂商锁定                | AWS 专有服务               | 开源数据库   |
> | 频繁需要 GSI/LSI            | 说明数据建模不适合 KV 结构 | 关系型数据库 |
>
> ------
>
> ## 十一、面试回答模板
...
> 
> 你答："我会用 DynamoDB，理由是：
> 1. 查询模式固定（按 [xxx] 查询），非常适合 KV 结构
> 2. 需要高可用和自动扩容
> 3. [Partition Key 选 xxx，Sort Key 选 xxx]，可以高效支持主要查询
> 4. 对于 [非主键查询场景]，我会建一个 GSI，以 [yyy] 为 partition key
> 5. 如果读多写少，可以加 DAX 缓存降低延迟
> 6. 如果需要同步其他系统，可以用 DynamoDB Streams 做 CDC"
> ```
>
> ------
>
...
> ```
> DynamoDB
> ├── 数据模型：Table → Item → Attribute（schema-less）
> ├── 主键：Partition Key（必须）+ Sort Key（可选）
> │   └── 底层：哈希分区 → B-Tree 排序
> ├── 查询：Query（高效）vs Scan（慢，避免）
> ├── 二级索引：
> │   ├── GSI：不同分区键，异步，随时建，最终一致
> │   └── LSI：相同分区键，同步，建表时建，可强一致
> ├── 一致性：最终一致（0.5RCU）vs 强一致（1RCU，只限主表+LSI）
> ├── 事务：✅ 支持 ACID，最多 100 items，可跨表
> ├── 扩展：每 partition 3000RCU + 1000WCU，超了自动分裂
> ├── 缓存：DAX（微秒延迟，不缓存强一致读）
> └── CDC：DynamoDB Streams → Lambda / Elasticsearch / Kinesis
> ```
>
> ------
>
> **记忆口诀**：**"分区排序建索引，强一致要找主节点，GSI 只能最终一致，事务 DAX Streams 加分项"** 🎯
...
>      
>  PgBouncer 维护一个预先建好的连接池，应用需要数据库连接时，从池子里借一个用完就还回去，PostgreSQL 始终只看到少量连接。
>  ① 应用发起请求，连接 PgBouncer（快，因为PgBouncer是轻量进程）
> 
> ② PgBouncer 检查连接池：
>    ├── 有空闲连接 → 立即分配
>    └── 没有空闲连接 → 应用等待（wait_timeout内）
> 
> ③ 应用通过这个连接执行SQL
> 
...
幂等性（Idempotence）是指在计算机、数学及业务系统中，用户对于同一个操作发起一次请求或多次请求，所产生的**最终影响（即资源状态）与执行一次的结果是一致的**，不会因重复操作而导致数据不一致。简单来说，就是“无论点击多少次，结果都一样”。

**核心要点：**

- **不仅是返回值：** 幂等性强调的是对系统状态的“副作用”一致，即使重复提交后，第一次请求可能成功，后续请求可能返回失败或不同的状态码（如 404），但系统数据只能被更新一次。
- **适用场景：** 主要用于解决网络重试、页面重复点击导致的重复支付、重复扣款、数据重复插入等风险。
- **常见实现方案：**
  - **数据库唯一主键：** 插入数据时利用主键冲突防止重复记录。
  - **业务唯一ID（Token机制）：** 每次请求带上唯一标识（如订单号），服务端利用Redis等校验该ID是否处理过。
  - 
  - **状态机：** 订单状态只能从“支付中”变更为“已支付”，不能从“已支付”再变回“已支付”，即状态的单向性。
...
> # Back-of-Envelope 必背数字完全指南
>
> ------
>
> ## 第一类：基础单位换算（所有计算的地基）
>
> 这类数字是"换算工具"，不对应任何架构组件，但所有估算都依赖它。
>
> | 单位 | 大小       | 记忆口诀     |
> | ---- | ---------- | ------------ |
> | KB   | 10³ bytes  | 一篇短文     |
...
> **时间单位：**
>
> - 1天 = 86,400秒 ≈ **10万秒**（面试直接用10^5）
> - 1月 ≈ 250万秒
> - 1年 ≈ 3000万秒
>
> ------
>
> ## 第二类：延迟数字（对应：架构选型 & 瓶颈分析）
>
> **用途：** 判断哪里是性能瓶颈，决定用缓存还是数据库，用内存还是磁盘。
>
> | 操作           | 延迟       | 记忆方法      |
> | -------------- | ---------- | ------------- |
> | L1 Cache       | 1 ns       | 闪电          |
> | L2 Cache       | 10 ns      |               |
> | RAM 读取       | 100 ns     |               |
> | SSD 随机读     | 100 μs     | 比RAM慢1000倍 |
> | HDD 随机读     | 10 ms      | 比SSD慢100倍  |
> | 同城网络RTT    | 1 ms       |               |
> | 跨大洲网络RTT  | 100-200 ms |               |
> | 磁盘顺序读 1MB | 1 ms       |               |
>
> **面试用法：**
>
> > "Redis是内存操作，延迟~100μs；如果用MySQL磁盘读，是10ms级别——差了100倍，所以热点数据必须上缓存。"
>
> **关键层级记忆：ns → μs → ms，每级差1000倍**
>
> ------
>
> ## 第三类：吞吐量 / QPS基准（对应：单机容量 & 何时需要扩容）
>
> **用途：** 判断单台服务器/数据库能撑多少流量，决定几台机器、是否需要分库分表。
>
> | 组件           | 典型吞吐量            | 备注           |
> | -------------- | --------------------- | -------------- |
> | 单台Web Server | **1万 QPS**           | Nginx/普通后端 |
> | MySQL 读       | **1000-5000 QPS**     | 加索引后       |
> | MySQL 写       | **1000 QPS**          | 瓶颈在磁盘     |
> | Redis          | **10万 QPS**          | 内存操作       |
> | Kafka          | **百万 msg/s**        | 顺序写磁盘     |
> | 单机网络带宽   | **1 Gbps ≈ 125 MB/s** |                |
>
> **面试用法：**
>
> > "我们估算峰值10万QPS，MySQL单机只能撑5000，所以需要20个读副本或分片。"
>
> ------
>
> ## 第四类：存储估算数字（对应：数据库选型 & 存储架构）
>
> **用途：** 估算需要多少存储，决定用什么数据库，是否需要分库分表、冷热分离。
>
> **常见数据大小：**
>
> | 数据类型         | 大小                  |
...
> **面试用法：**
>
> > "DAU 1亿，每人每天发1条消息，每条1KB → 1亿KB = 100GB/天 → 5年约180TB，需要分布式存储。"
>
> ------
>
> ## 第五类：用户规模基准（对应：整体架构规模判断）
>
> **用途：** 把"用户数"转换成"QPS"，这是所有估算的起点。
>
> **核心公式：**
>
> ```
> QPS = DAU × 每人每天操作次数 ÷ 86400秒
> 峰值QPS = 平均QPS × 2~3倍
> ```
>
> **常用换算经验：**
>
> | DAU    | 平均QPS（每人10次操作） | 峰值QPS |
> | ------ | ----------------------- | ------- |
> | 100万  | ~115 ≈ **1千**          | 3千     |
> | 1000万 | ~1150 ≈ **1万**         | 3万     |
> | 1亿    | ~11500 ≈ **10万**       | 30万    |
> | 10亿   | ~115000 ≈ **100万**     | 300万   |
>
> **记忆口诀：DAU每增加10倍，QPS增加10倍**
>
> ------
>
> ## 第六类：可用性数字（对应：SLA & 冗余架构设计）
>
> **用途：** 说明系统可用性目标，解释为什么需要多副本、多机房。
>
> | SLA             | 年宕机时间 | 意味着   |
> | --------------- | ---------- | -------- |
> | 99%             | 87小时     | 不可接受 |
...
> **面试用法：**
>
> > "我们目标四个9，单机宕机概率1%，所以需要至少3副本 + 跨AZ部署。"
>
> ------
>
> ## 🎯 不同规模推荐架构速查表
...
> | 规模               | QPS       | 推荐方案                                                     |
> | ------------------ | --------- | ------------------------------------------------------------ |
> | **小型** 10万DAU   | <1000 QPS | 单体应用 + MySQL主从 + Redis缓存                             |
> | **中型** 百万DAU   | 1万 QPS   | 微服务 + MySQL读写分离 + Redis集群 + CDN                     |
> | **大型** 千万DAU   | 10万 QPS  | 服务拆分 + MySQL分库分表（或换DynamoDB）+ 多层缓存 + Kafka异步 |
> | **超大型** 亿级DAU | 100万 QPS | 全分布式 + NoSQL为主 + 全球多区域部署 + 流计算               |
>
> ------
>
> ## 面试开场万能模板
>
> ```
> 1. 确认DAU → 算平均QPS → 算峰值QPS
> 2. 确认读写比例（通常读多写少，如10:1）
> 3. 估算存储（单条大小 × 每天新增量 × 保留年限）
> 4. 根据QPS判断单机能否撑住 → 决定扩容策略
> 5. 根据存储量决定数据库类型和分片方案
> ```
>
> **这5步做完，你的规模感就建立好了，后面所有架构决策都有数字支撑。**
...
>
> - **分布式 (Distributed)**
>   - **概念**：将一个复杂业务拆分为多个子任务（模块），由不同节点（服务器）分别处理。
>   - **特点**：模块解耦，高扩展性，适合缩短大任务执行时间。
>   - **场景**：微服务架构、大数据计算（如Spark）、分布式数据库。
> - **集群 (Cluster)**
>   - **概念**：多台服务器集中在一起，部署相同的服务，对外提供统一的业务。
>   - **特点**：通过负载均衡（LB）将请求分发至不同节点，提高系统并发能力和高可用性（一机宕机，另机顶上）。
>   - **场景**：Web服务器集群、数据库读写分离集群。 
>
> 分布式与集群的异同点
>
> | 特性           | 分布式                         | 集群                         |
> | :------------- | :----------------------------- | :--------------------------- |
> | **工作方式**   | 分工协作（分头做事）           | 合作冗余（一堆人做一事）     |
> | **目的**       | 降低复杂性、提高性能、缩短时长 | 高可用、负载均衡、提高吞吐量 |
> | **业务相同性** | 不同节点部署不同服务           | 不同节点部署相同服务         |
> | **扩展方式**   | 纵向拆分业务（垂直扩展）       | 横向增加机器（水平扩展）     |
> | **节点关系**   | 松散                           | 紧密（通常由主节点管理）     |
>
> 举例理解
>
> - **饭店炒菜（集群）**：为了应付更多顾客，原本1个厨师增加到5个厨师，这5个厨师都会做炒饭。顾客点了炒饭，谁不忙谁做。
> - **饭店炒菜（分布式）**：为了提高炒菜效率，不仅有厨师（处理请求），还聘请了专门的切菜师、洗菜师、配菜师，各司其职。 
>
> 总结
>
> 1. **分布式可以包含集群**：分布式系统中的某一个节点，可以为了高可用而做成集群。
> 2. **集群不一定是分布式**：仅仅是将多个相同的节点组合在一起。
> 3. **核心联系**：两者都用于提高系统的整体能力，实现高可用和高性能。
